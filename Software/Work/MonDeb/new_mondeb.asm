...
; This is adapted from Jeff Tranter's work, and is a port of MONDEB to
; my HD6309 SBC. Jeff's work can be found here:
; https://github.com/jefftranter/6809/blob/master/sbc/mondeb/mondeb.asm
;
; This code uses SCC UART functions that are already present elsewhere 
; in this ROM. I've migrated the syntax to that of William Astle's
; LWTOOLS toolchain, which can be found here: http://www.lwtools.ca/
;
...
;; MONDEB - primary entry point
MONDEB: 
 		LDS	 	#INITSTACK			; grow stack downward from reserved RAM 
		LDU		#MONDEB_RAM			; point U at beginning of MONDEB_RAM block
		
;; MONDEB2 - alternate entry point
;; inputs:  U = pointer to beginning of 240 bytes of RAM
;;			S = stack pointer already established!
MONDEB2:							
		STS		SP					; save the stack pointer
		JSR		INITIAL				; initialize variables and SCC
...
JMPCMD 	TFR    	A,B      			;SAVE COMMAND # IN ACCB
		ADDR	A,B					;B contains A*2
		ADDR	A,B					;B contains A*3
		LDX		#JMPTBL		        ;ADD B TO BASE OF JUMP TABLE 
		ABX							;ABX is unsigned addition
...
T?1 	DEC		12,X      		;DECR LO BYTE OF PC
								; fall into DISREG...
...
;TYPE THE STACK POINTER LOCATION
		...
		LBRA	NOMORE
;OUTPUT name and CONTENT OF A 1 BYTE REGISTER
OUT2	BSR		OUT2A4
    	JSR		OUT1BY
    	LEAX   	1,X				; advance to next byte
    	RTS
;OUTPUT name and CONTENT OF A 2 BYTE REGISTER
OUT4	BSR    	OUT2A4
    	JSR    	OUT2BY
...
OUT2A4	JSR		OUTSP    		;OUTPUT A SPACE
		...
		JSR		TYPCMD   		;TYPE IT
		JMP		OUTEQ    		;TYPE AN "=", return

;*****
;GOTO - GO TO MEMORY ADDRESS
GOTO	LDX		LASTGO			;get last GOTO address (as default)
		JSR		NUMBER   		;GET DESTINATION
		BEQ		G?O1    		;IF NONE, USE DEFAULT
		LDX		NBRHI			;get specified address
		STX		LASTGO   		;SAVE IT
G?O1	JMP		,X       		;GO TO DESTINATION
...
COPY	JSR		GTRANG			;GET SOURCE RANGE INTO RANGLO & RANGHI
		LBLE	BADSYN 			;syntax ERROR IF NO SOURCE
		JSR		NUMBER			;GET DESTINATION
		LBLE	BADSYN 			;syntax ERROR IF NO DESTINATION
		LDX		RANGLO			;GET SOURCE ADDRESS POINTER
C?1		LDA		,X				;GET BYTE FROM SOURCE
		LDX 	NBRHI    		;GET DESTINATION ADDRESS POINTER
		STA 	,X+       		;SAVE BYTE IN DESTINATION, inc pointer
		STX 	NBRHI    		;SAVE IT
		LDX 	RANGLO   		;GET SOURCE ADDRESS POINTER
...
;original MONDEB is ^D (4) terminator, but common PUTSTR uses NULL terminator!
;ACCA & ACCB ARE PRESERVED
;IX IS LEFT POINTING TO THE STRING TERMINATOR
OUTSTR	EQU		PUTSTR          ; same functionality as PUTSTR

;======================================================
;INPUT A CHARACTER FROM SCC CHANNEL A
;ACCB & IX ARE PRESERVED
INPCHR 	EQU		GETA			; same functionality as GETA

;======================================================
;OUTPUT THE CHARACTER IN ACCA TO THE DESIRED OUTPUT DEVICE/LOCATION
; IF OUTFLG = 0/1, OUTPUT IS TO SCC CHANNEL A
; IF OUTFLG = 2, OUTPUT IS TO ADDRESS IN OUTADR & THIS ADDR IS THEN INCREMENTED
;ACCA, ACCB, & IX ARE PRESERVED
OUTCHR	PSHS   	B,X    			;SAVE ACCB and X
		LDX 	OUTADR   		;GET OUTPUT CHAR DESTINATION ADDRESS
		LDB 	#2
		CMPB	OUTFLG
		BNE		O?CH4   		;handle dump to memory
;OUTFLG = 2: MEMORY OUTPUT
O?CH2 	STA    	,X+				;SAVE CHAR IN MEMORY and increment
		STX		OUTADR			;save UPDATED OUTPUT ADDRESS
O?CH3 	PULS   	B,X,PC     		;RESTORE ACCB and x and return
;OUTFLG = 0: TERMINAL ACIA OUTPUT
;IGNORE LINE FEEDS
O?CH4	CMPA	#LF				;is it LF?
		BEQ		O?CH3			;	yes, ignore it return
O?CH5	CMPA   	#CR				;is it CR?
		BNE    	O?CH6			;	no, handle it
		BSR    	DOCRLF			;	yes, output CR+LF
		BRA		O?CH3			;and then return
O?CH6 	LDB    	CPLCNT   		;GET "CHARACTERS PER LINE" COUNT
		CMPB	CPLMAX   		;COMPARE TO MAX PERMISSIBLE
		BGE 	O?CH7   		;SEND CR-LF IF GREATER
;LESS THAN MAX, BUT ALSO SEND CR-LF IF 10 FROM END AND PRINTING A SPACE
		ADDB   	#10
		CMPB   	CPLMAX          ;is 10 more than CPLCNT > MAX?
		BLT    	O?CH8          	;	nope, skip CR-LF injection
		CMPA   	#SPACE     		;NEAR END, TEST IF ABOUT TO PRINT A SPACE
		BNE    	O?CH8			;   nope, skip CR-LF injection
;TERMINAL LINE FULL OR NEARLY FULL - INTERJECT A CR-LF
O?CH7 	BSR    	DOCRLF
O?CH8 	INC    	CPLCNT   		;BUMP COUNTER
       	JSR		PUTA			;send to SCC channel A
		BRA		O?CH3			;and then return

;======================================================
;SEND A CARRIAGE RETURN-LINE FEED TO THE TERMINAL
;ACCA, ACCB, & IX ARE PRESERVED
DOCRLF	JSR		PUTCR			; output CR+LF 
...
;======================================================
;MISC TEST
MSGHED	FCC		"MONDEB 6809 1.02 30-Mar-2020" ;MONITOR HEADER TYPEOUT
      	FCB		$FF
      	    		
MSGPRM	FCN		"*"        			;PROMPT STRING
      	    		
MSGSWI	FCB		CR
      	FCN		"SWI:"				; ?
      	   		
MSGLTL	FCN		"TOO LONG" 			; TYPED IF INPUT LINE IS TOO LONG
      	   		
MSGNBR	FCN		"NOT SET" 			; BREAK NOT SET
      	   		
MSGBAT	FCN		"SET @ " 			; BREAK AT -
      	   		
MSGVER	FCN		"OK"     			;CHECKSUM VERIFIES
      	   		
MSGNVE	FCN		"CHECKSUM ERROR " 	; FOR VERIFY & LOAD COMMANDS
      	   		
MSGCCL	FCN		"CAN'T CLEAR" 		; TEST COMMAND
      	   		
MSGCSO	FCN		"CAN'T SET TO ONES" ; TEST COMMAND
      	   		
MSGSIS	FCN		"SUM IS " 			; COMPARE COMMAND
      	   		
MSGDIS	FCN		", DIFF IS " 		; COMPARE COMMAND

MSGS0 	FCB		CR,LF
      	FCN		"S00600004844521B"  ; S0 opener
      	    		
MSGS1 	FCB		CR,LF
      	FCN		"S1"				; S1 start of record
      	    		
MSGS9 	FCB		CR,LF
      	FCC		"S9030000FC"		; S9 closer
      	FCB		$FF
